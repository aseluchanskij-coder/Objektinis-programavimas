...
int random_paz() { return rand() % 10 + 1; }

string random_vardas() {
    string v[10] = {"Jonas","Petras","Mantas","Lukas","Tomas","Paulius","Alanas","Danielius","Povilas","Ernestas"};
    return v[rand() % 10];
}

string random_pavarde() {
    string p[10] = {"Jonaitis","Petraitis","Kazlauskas","Balciunas","Jankauskas","Stankevicius","Vaitkus","Butkus","Urbonas","Zukauskas"};
    return p[rand() % 10];
}
...
void prideti(studentas *&grupe, int &m, studentas &A) {
    studentas *naujas = new studentas[m + 1];

    for (int i = 0; i < m; i++) {
        naujas[i] = grupe[i];
        naujas[i].paz = new int[grupe[i].paz_kiek];
        for (int j = 0; j < grupe[i].paz_kiek; j++)
            naujas[i].paz[j] = grupe[i].paz[j];
    }

    naujas[m] = A;
    naujas[m].paz = new int[A.paz_kiek];
    for (int j = 0; j < A.paz_kiek; j++)
        naujas[m].paz[j] = A.paz[j];
...
void input(studentas *&grupe, int &m) {
    ...
    while (true) {
        ...
        if (menu == 3) {
            ...
            string bendrasTipas;
            cout << "Skaiciuoti su vid ar med? ";
            cin >> bendrasTipas;
            while (bendrasTipas != "vid" && bendrasTipas != "med") {
                cout << "Klaida! Iveskite 'vid' arba 'med': ";
                cin >> bendrasTipas;
            }
        ...
        if (menu == 1) {
            cout << "Iveskite pazymius (1-10, 0 - baigti): ";
            while (true) {
